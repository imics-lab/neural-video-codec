...
class DMCI(CompressionModel):
    ...
    def forward_train(self, x: torch.Tensor, qp: int, use_qat: bool = False):
        """
        Training forward pass for DMCI (I-frame model).

        When ``use_qat=True``, Conv2d layers use INT16 fake-quant (STE) and
        latents use round-clamp instead of uniform noise.
        """
        if use_qat:
            enable_qat(self)
        ...
        try:
            ...
            q_enc_h, q_dec_h, scales, means = self.separate_prior(params, is_video=False)
