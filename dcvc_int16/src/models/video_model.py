...
from ..training.fake_quant import fake_quant_feature, fake_quant_latent
from ..training.qat_hooks import disable_qat, enable_qat
...
class DMC(CompressionModel):
    ...
    # ------------------------------------------------------------------
    # Training forward pass (differentiable, no rANS)
    # ------------------------------------------------------------------
    ...
    def forward_train(
        ...
        """
        Training forward pass for DMC (P-frame model).

        Args:
            detach_ref: when True (default), DPB tensors are detached after update
                        (truncated BPTT). Set False for short-horizon full BPTT.
        """
        if use_qat:
            enable_qat(self)
            x = fake_quant_feature(x)
        ...
        try:
            ...
            if use_qat:
                z_hat = fake_quant_latent(z)
            else:
                z_hat = z + (torch.rand_like(z) - 0.5)

            qp_idx = torch.full((B,), qp, device=z.device, dtype=torch.int32)
            z_upper = self.bit_estimator_z.get_cdf(z_hat + 0.5, qp_idx)
            z_lower = self.bit_estimator_z.get_cdf(z_hat - 0.5, qp_idx)
            z_bits = -torch.log2((z_upper - z_lower).clamp_min(1e-9)).sum()

            params = self.res_prior_param_decoder(z_hat, ctx_t)
            params = params[:, :, :yH, :yW].contiguous()

            q_dec_prior, scales, means = params.chunk(3, 1)
            q_dec_prior = q_dec_prior.clamp_min(0.5)
            q_enc_prior = 1.0 / q_dec_prior

            y_scaled  = y * q_enc_prior
            y_res     = y_scaled - means
            if use_qat:
                y_res_hat = fake_quant_latent(y_res)
            else:
                y_res_hat = y_res + (torch.rand_like(y_res) - 0.5)

            dist   = torch.distributions.Normal(0.0, scales.clamp(min=0.01))
            y_bits = -dist.log_prob(y_res_hat).sum() / math.log(2)

            y_hat   = (y_res_hat + means) * q_dec_prior
            feature = self.decoder(y_hat, ctx, q_decoder)
            x_hat   = self.recon_generation_net(feature, q_recon)

            if detach_ref:
                self.add_ref_frame(feature.detach(), x_hat.detach())
            else:
                self.add_ref_frame(feature, x_hat)
        finally:
            if use_qat:
