...
write_kept_frames_video: Optional[Callable[..., Dict[str, Any]]] = None
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config",
        type=str,
        default=str(ROOT / "configs" / "compression.yaml"),
...
def _select_roi_model_for_runtime(cfg: Dict[str, Any]) -> Dict[str, Any]:
    """
    Use the ONNX detector model automatically when it is present and a GPU ONNX
    Runtime provider is available. Otherwise fall back to the default PyTorch model.
...
def _apply_runtime_device_fallbacks(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    report["dcvc_device_selected"] = str(sel_label)
...
def main() -> None:
    ...
    archive_payloads = {
        str(out_cfg.get("roi_json", "roi_detections.json")): json.dumps(roi_result, indent=2).encode("utf-8"),
        str(out_cfg.get("frame_drop_json", "frame_drop.json")): json.dumps(frame_result, indent=2).encode("utf-8"),
        str(out_cfg.get("roi_bin", "roi.bin")): bytes(compression_result["roi_bin_bytes"]),
        str(out_cfg.get("bg_bin", "bg.bin")): bytes(compression_result["bg_bin_bytes"]),
        str(out_cfg.get("dcvc_meta", "meta.json")): json.dumps(compression_result["meta"], indent=2).encode("utf-8"),
    }
