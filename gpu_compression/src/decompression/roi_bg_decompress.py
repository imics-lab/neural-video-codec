...
Final timeline interpolation and ROI/BG compositing are handled by run_decompression.py.
...
def _configure_cuda(use_cuda: bool, cuda_idx: Any) -> None:
    ...
    if cuda_idx is None:
        return
    if isinstance(cuda_idx, (list, tuple)):
        os.environ["CUDA_VISIBLE_DEVICES"] = ",".join(str(x) for x in cuda_idx)
    else:
...
def _decode_stream_bytes_dcvc(
    ...
    _configure_cuda(use_cuda, cuda_idx)
