...
def remove_redundant_frames(video_path: str, bbox_map_raw: Dict[int, List[Any]], cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    p_roi = params.get("roi", {}) or {}
    halo = float(p_roi.get("halo_frac", 0.2))
    fixed_size = int(p_roi.get("fixed_size", 160))
    blur_ksize = int(p_roi.get("blur_ksize", 5))
    to_gray = bool(p_roi.get("gray", True))

    p_smooth = params.get("bbox_smoothing", {}) or {}
    smooth_enable = bool(p_smooth.get("enable", True))
    ema_alpha = float(p_smooth.get("ema_alpha", 0.6))

    p_motion = params.get("motion", {}) or {}
    pixel_t_low = float(p_motion.get("t_low", 0.9))
    pixel_t_high = float(p_motion.get("t_high", 1.9))
    ...
    bbox_t_low_px = float(p_motion.get("bbox_t_low_px", 0.5))
    bbox_t_high_px = float(p_motion.get("bbox_t_high_px", 1.0))
    if bbox_t_low_px < 0.0 or bbox_t_high_px < 0.0:
        raise ValueError("motion.bbox_t_low_px and motion.bbox_t_high_px must be >= 0")
    if bbox_t_high_px <= bbox_t_low_px:
        raise ValueError("motion.bbox_t_high_px must be > motion.bbox_t_low_px")

    enter_motion_frames = int(p_motion.get("enter_motion_frames", 2))
    enter_still_frames = int(p_motion.get("enter_still_frames", 4))
    if enter_motion_frames < 1 or enter_still_frames < 1:
        raise ValueError("motion.enter_motion_frames and motion.enter_still_frames must be >= 1")

    dual_cfg = cfg.get("dual_timeline", {}) or {}
    roi_motion_interval = int(dual_cfg.get("roi_motion_interval", 2))
    roi_still_interval = int(dual_cfg.get("roi_still_interval", 3))
    if roi_motion_interval < 1 or roi_still_interval < 1:
        raise ValueError("dual_timeline.roi_motion_interval and dual_timeline.roi_still_interval must be >= 1")
