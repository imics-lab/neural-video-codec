...
We evaluate on a held-out set of 1080p wildlife camera trap recordings captured in a ranch environment, totalling approximately \TODO{X hours, Y videos}. Videos contain deer, coyotes, and domestic cattle recorded by fixed cameras at dawn, midday, and dusk. Approximately \TODO{Z\%} of frames contain at least one animal.

For training RestoreUNet, DCVC compression pairs are generated on a separate training split using the same pipeline configuration as inference (QP$_{\text{ROI}}{=}63$, QP$_{\text{BG}}{=}5$, reset interval 32). The evaluation set is held out entirely from training data generation.
...
Table~\ref{tab:roi_bg} reports separate ROI and background PSNR for our pipeline as we vary QP$_{\text{BG}}$ from 5 to 40. As expected, ROI quality is nearly independent of QP$_{\text{BG}}$ since the animal region is always encoded at QP$_{\text{ROI}}{=}63$. The background quality degrades gracefully; with RestoreUNet the gap to the uncompressed background closes considerably at all tested QP$_{\text{BG}}$ values.

\begin{table}[t]
\centering
\caption{Effect of background QP on per-region quality. ROI PSNR is stable across QP$_\text{BG}$ settings; RestoreUNet closes the gap on background.}
\label{tab:roi_bg}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{lcccc}
\toprule
QP$_{\text{BG}}$ & \multicolumn{2}{c}{PSNR (ROI / BG) w/o rest.} & \multicolumn{2}{c}{PSNR (ROI / BG) w/ rest.} \\
\midrule
5  & \TODO{XX.X} / \TODO{XX.X} & & \TODO{XX.X} / \TODO{XX.X} & \\
15 & \TODO{XX.X} / \TODO{XX.X} & & \TODO{XX.X} / \TODO{XX.X} & \\
25 & \TODO{XX.X} / \TODO{XX.X} & & \TODO{XX.X} / \TODO{XX.X} & \\
40 & \TODO{XX.X} / \TODO{XX.X} & & \TODO{XX.X} / \TODO{XX.X} & \\
\bottomrule
\end{tabular}
\end{table}
...
We evaluate RestoreUNet with temporal windows $T \in \{1, 3, 5\}$, keeping all other settings equal. $T{=}1$ corresponds to per-frame restoration without temporal attention; $T{=}3$ is the proposed setting. Results in Table~\ref{tab:ablation_T} show that temporal attention consistently improves SSIM and LPIPS, with $T{=}3$ offering the best quality-speed trade-off.
...
\midrule
1 (no temporal) & \TODO{X.X} & \TODO{.XXX} & \TODO{.XXX} \\
3 (proposed)    & \TODO{X.X} & \TODO{.XXX} & \TODO{.XXX} \\
5               & \TODO{X.X} & \TODO{.XXX} & \TODO{.XXX} \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{DDIM steps.}
Table~\ref{tab:ablation_steps} reports quality versus inference time as we vary the number of DDIM steps $d \in \{1, 3, 6, 10, 20\}$. At $t_s{=}200$ (only 20\% of the full diffusion trajectory is traversed), quality saturates quickly; $d{=}6$ achieves near-peak PSNR at significantly reduced compute compared to $d{=}20$.

\begin{table}[t]
\centering
\caption{Ablation: DDIM steps $d$ vs.\ quality and inference time (20-second test clip on one NVIDIA RTX PRO 6000).}
\label{tab:ablation_steps}
\begin{tabular}{lcccc}
\toprule
$d$ & PSNR (dB) & SSIM & LPIPS & Time (s) \\
\midrule
1  & \TODO{X.X} & \TODO{.XXX} & \TODO{.XXX} & \TODO{XX} \\
3  & \TODO{X.X} & \TODO{.XXX} & \TODO{.XXX} & \TODO{XX} \\
6  & \TODO{X.X} & \TODO{.XXX} & \TODO{.XXX} & \TODO{XX} \\
10 & \TODO{X.X} & \TODO{.XXX} & \TODO{.XXX} & \TODO{XX} \\
20 & \TODO{X.X} & \TODO{.XXX} & \TODO{.XXX} & \TODO{XX} \\
\bottomrule
\end{tabular}
\end{table}
...
At $d{=}6$ DDIM steps and $t_s{=}200$, RestoreUNet processes a 20-second video in approximately 3 minutes on a single RTX PRO 6000 (without tiling, as the 96\,GB GPU fits 1080p full-frame). With tiling enabled (required on consumer GPUs), all tiles are batched into a single DDIM call per frame, limiting the overhead to a small constant factor over the untiled path.
