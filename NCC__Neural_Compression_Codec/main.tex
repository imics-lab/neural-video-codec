...
We evaluate RestoreUNet with temporal windows $T \in \{1, 3, 5\}$, keeping all other settings equal. $T{=}1$ reduces to per-frame restoration with no temporal attention; $T{=}3$ is the proposed setting. Because PSNR, SSIM, and LPIPS are computed independently on each output frame, they are insensitive to $T$ by construction; the numbers in Table~\ref{tab:ablation_T} are within measurement noise across all three settings. The benefit of temporal attention is flicker suppression and cross-frame coherence, which require video-level viewing to assess. We select $T{=}3$ as the default: it provides temporal context without the additional memory and compute cost of $T{=}5$.

\begin{table}[t]
\centering
\caption{Ablation: effect of temporal window size $T$. Per-frame metrics are insensitive to $T$ by construction; differences are within measurement noise.}
\label{tab:ablation_T}
\begin{tabular}{lccc}
\toprule
$T$ & PSNR (dB) & SSIM & LPIPS \\
\midrule
1 (no temporal) & 28.79 & 0.8338 & 0.3116 \\
3 (proposed)    & 28.83 & 0.8314 & 0.3234 \\
